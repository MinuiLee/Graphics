...
void Mesh::SetNormals()
{
	...
	for (size_t i = 0; i < nrOfVertices; ++i)
	{
		...
		normal_lines.push_back({ vertexArray[i].position + vertexArray[i].normal * 0.005f, glm::vec3{ 0.f,0.f,1.f } });
	}
	...
	for (size_t i = 0; i < nrOfIndices; i += 3)
	{
		...
		fnormal_lines.push_back({ fCenter + face_normals[i] * 0.005f, glm::vec3{ 0.f,1.f,0.f } });
	}
}
...
void Mesh::ResetVertices()
{
	...
	for (size_t i = 0; i < nrOfVertices; ++i)
	{
		...
		//cal min max
		min = glm::min(vertexArray[i].position, min);
		max = glm::max(max, vertexArray[i].position);
	}
}
