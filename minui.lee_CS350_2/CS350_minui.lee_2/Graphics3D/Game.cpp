	...
	drawNormal(false), drawFaceNormal(false)
{
	...
	tests[0] = "Sphere vs Sphere";
	tests[1] = "AABB vs Sphere";
	tests[2] = "Sphere vs AABB";
	tests[3] = "AABB vs AABB";
	tests[4] = "Point vs Sphere";
	tests[5] = "Point vs AABB";
	tests[6] = "Point vs Triangle";
	tests[7] = "Point vs Plane";
	tests[8] = "Ray vs Plane";
	tests[9] = "Ray vs AABB";
	tests[10] = "Ray vs Sphere";
	tests[11] = "Ray vs Triangle";
	tests[12] = "Plane vs AABB";
	tests[13] = "Plane vs Triangle";
}
...
void Game::Render()
{
	...
	case 0:
	{
		for (int i = 0; i < 5; ++i)
		{
			...
			for (int j = 0; j < 5; ++j)
			{
				if (i != j && bSpheres[i].TestSphereVsSphere(bSpheres[j]))
				{
					intersect = true;
					break;
				}
			}
			if (intersect) bSpheres[i].Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else bSpheres[i].Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}
	case 1:
	{
		for (BoundingSphere& s : bSpheres) s.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		for (AABB& a : aabbs)
		{
			bool intersect = false;
			for (BoundingSphere& s : bSpheres)
			{
				if (a.TestAABBvsSphere(s))
				{
					intersect = true;
					break;
				}
			}
			if (intersect) a.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else a.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}
	case 2:
	{
		for (AABB& a : aabbs) a.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		for (BoundingSphere& s : bSpheres)
		{
			bool intersect = false;
			for (AABB& a : aabbs)
			{
				if (s.TestSphereVsAABB(a))
				{
					intersect = true;
					break;
				}
			}
			if (intersect) s.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else s.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}
	case 3:
	{
		for (int i = 0; i < 5; ++i)
		{
			...
			for (int j = 0; j < 5; ++j)
			{
				if (i != j && aabbs[i].TestAABBvsAABB(aabbs[j]))
				{
					intersect = true;
					break;
				}
			}
			if (intersect) aabbs[i].Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else aabbs[i].Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}case 4:
	{
		...
		for (Point3D& p : points)
		{
			...
			for (BoundingSphere& s : bSpheres)
			{
				if (p.PointVsSphere(s))
				{
					intersect = true;
					break;
				}
			}
			if (intersect) p.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else p.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}case 5:
	{
	...
	{
		for (Triangle& t : triangles) t.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		for (Point3D& p : points)
		{
			bool intersect = false;
			for (Triangle& t : triangles)
			{
				if (p.PointVsTriangle(t))
				{
					intersect = true;
					break;
				}
	...
	{
		for (Plane& p : planes) p.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		for (Point3D& p : points)
		{
			bool intersect = false;
			for (Plane& pl : planes)
			{
				if (p.PointVsPlane(pl))
				{
					intersect = true;
					break;
				}
	...
	}case 8:
	{
		...
		for (Ray& r : rays)
		{
			...
			if (intersect) r.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 0.f, 0.f));
			else r.Draw(shaders[SHADER_BASIC], glm::vec3(1.f, 1.f, 1.f));
		}
		break;
	}
	case 9:
...
void Game::UpdateImGUI()
{
	...
	{
		if(ImGui::Button("New Primitives"))
		{
			InitPrimitives();
		}
		ImGui::Text("Tests");
		ImGui::ListBox("", &mode, tests, IM_ARRAYSIZE(tests));
...
void Game::GeomatryPass()
{
	...
	{
		centerObject->Render(shaders[SHADER_GBUFFER], meshes[i]);
	}
...
void Game::InitPrimitives()
{
	triangles[0].Initialize(glm::vec3(-1, 1, 1), glm::vec3(0, -1, 1), glm::vec3(1, 1, 1));
	triangles[1].Initialize(glm::vec3(-1, 2, 0), glm::vec3(2, 2, -1), glm::vec3(2, 2, 0));
	triangles[2].Initialize(glm::vec3(1, 0, 1), glm::vec3(1, 0, 0), glm::vec3(0, 0, 1));
	
	planes[0].Initialize(glm::vec3(Random(1), Random(1), Random(1)), Random(5));
	rays[0].Initialize(glm::vec3(Random(2), Random(2), Random(2)), glm::vec3(Random(1), Random(1), Random(1)));

	for (int i = 0; i < 5; ++i)
	{
		aabbs[i].Initialize(glm::vec3(Random(4), Random(4), Random(4)), glm::vec3(Random(4), Random(4), Random(4)));

		bSpheres[i].Initialize(glm::vec3(Random(4), Random(4), Random(4)), rand() % 20 / 10.f);
		
		
		points[i*2].Initialize(glm::vec3(Random(3), Random(3), Random(3)));
		points[i*2+1].Initialize(glm::vec3(Random(3), Random(3), Random(3)));
	}
}

float Game::Random(int range)
{
	range *= 10;
	return (rand() % range - (range/2)) / 10.f;
}
