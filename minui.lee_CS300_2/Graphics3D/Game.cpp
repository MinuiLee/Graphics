...
Game::Game(const char* title) :
	...
	camera(glm::vec3(0.f, 2.f, 10.f), glm::vec3(0.f, 0.f, 1.f), glm::vec3(0.f, 1.f, 0.f)),
	camPosition(glm::vec3(0.f, 2.f, 10.f)),
...
void Game::Render()
{
	...
	for (int i = 0; i < numLights; ++i)
	{
		if (lightObjs[i]->GetCanRotate())
		{
			...
			glm::vec3 d(planeObj->GetRawPosition() - lightObjs[i]->GetRawPosition());
			d.x *= 1.5f;
			d.z *= 1.5f;
			lights[i]->SetDirection(d);
		}
		lightObjs[i]->Render(shaders[SHADER_BASIC], lightMesh);
	}

	centraObj->SetLight(lights);
	planeObj->SetLight(lights);

	centraObj->Render(shaders[shaderType], meshes[centraObj->GetMeshName()]);
	planeObj->Render(shaders[shaderType], meshes[planeObj->GetMeshName()]);
...
void Game::UpdateImGUI()
{
	...
	if (ImGui::CollapsingHeader("\"_____\""))
	{
...
void Game::InitModels()
{
	...
	centraObj = new Object(glm::vec3(0.f), materials[0], lights, textures[TEX_DIFF], textures[TEX_SPEC], meshNames.front());
	...
	planeMesh = new Mesh(new Quad());
	planeObj = new Object(glm::vec3(0.f, 0.f, 0.f), materials[0], lights, textures[TEX_DIFF], textures[TEX_SPEC], "OBJ_quad");
	planeObj->Scale(glm::vec3(5.f));
	planeObj->Rotate(glm::vec3(-90.f, 0.f, 0.f));
	planeObj->SetPosition(glm::vec3(0.f, -1.5f, 0.f));
}
