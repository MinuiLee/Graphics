...
void Mesh::SetNormals()
{
	...
	for (size_t i = 0; i < nrOfVertices; ++i)
	{
		...
		normal_lines.push_back({ vertexArray[i].position + vertexArray[i].normal * 0.1f, glm::vec3{ 0.f,0.f,1.f } });
	}
	...
	for (size_t i = 0; i < nrOfIndices; i += 3)
	{
		...
		fnormal_lines.push_back({ fCenter + face_normals[i] * 0.1f, glm::vec3{ 0.f,1.f,0.f } });
	}
}
...
void Mesh::SetOrbitPoints()
{
	...
	for (float i = 0; i < div; i += 2.f)
	{
		orbit_lines.push_back({ glm::vec3{ dis * cosf(i * step), 0.f, dis * sinf(i * step)}, glm::vec3{0.f,1.f,0.f} });
		if (i == div - 1)
			orbit_lines.push_back({ glm::vec3{ dis * cosf(0), 0.f, dis * sinf(0)}, glm::vec3{0.f,1.f,0.f} });
		else
			orbit_lines.push_back({ glm::vec3{ dis * cosf((i + 1) * step), 0.f, dis * sinf((i + 1) * step)}, glm::vec3{0.f,1.f,0.f} });
	}
}
